...
(function () {
  ...
  loadFB(function() {
    try {
      ...
      // Otomatis suntikkan SVG dari konfigurasi ke elemen HTML jika tersedia
      if (conf.viewIcon) {
        const vContainer = document.querySelector(".aDv-container");
        if (vContainer) {
          let existingSvg = vContainer.querySelector("svg");
          if (existingSvg) existingSvg.outerHTML = conf.viewIcon;
        }
      }

      if (conf.clapIcon && btn) {
        let existingSvgBtn = btn.querySelector("svg");
        if (existingSvgBtn) existingSvgBtn.outerHTML = conf.clapIcon;
      }
      ...
      viewRef.on("value", snap => { 
        if(vEl) vEl.innerText = formatNum(snap.val() || 0); 
      });
      ...
      window.triggerClap = function() {
        ...
        const maxLimit = Number(liveConf.maxLimit) || 10;
