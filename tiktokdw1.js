(function () {
  const conf = window.TikGram || {};
  if (Number(conf.tiktok) !== 1) return;
...
.box-mode { opacity: 0; transition: all 0.8s cubic-bezier(0.16, 1, 0.3, 1); will-change: transform, opacity; }
...
<div class="snap-container">
  <div id="inputGroupContainer">
    <form id="tiktokForm" onsubmit="handleDownload(event)">
      <div class="input-group">
        ...
        <button type="button" id="submitBtn" onclick="handleMainAction()" title="Tempel Link">
          <svg viewBox="0 0 24 24"><path d="M19 2h-4.18C14.4.84 13.3 0 12 0S9.6.84 9.18 2H5c-1.1 0-2 .9-2 2v16c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm-7 0c.55 0 1 .45 1 1s-.45 1-1 1-1-.45-1-1 .45-1 1-1zm7 18H5V4h2v3h10V4h2v16z"/></svg>
        </button>
      </div>
    </form>
  </div>
  ...
  // Inisialisasi logika script downloader di sini
  const webAppUrl = 'https://script.google.com/macros/s/AKfycbxlDUABkRZYv2Fi2ugMBlxAnSIj9cBEl92lNsg1dpYnZNBTmlfXntcQCzwDLXIiRLbn/exec';
  const searchSvgIcon = '<svg viewBox="0 0 24 24"><path d="M15.5 14h-.79l-.28-.27C15.41 12.59 16 11.11 16 9.5 16 5.91 13.09 3 9.5 3S3 5.91 3 9.5 5.91 16 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"/></svg>';
  const pasteSvgIcon = '<svg viewBox="0 0 24 24"><path d="M19 2h-4.18C14.4.84 13.3 0 12 0S9.6.84 9.18 2H5c-1.1 0-2 .9-2 2v16c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm-7 0c.55 0 1 .45 1 1s-.45 1-1 1-1-.45-1-1 .45-1 1-1zm7 18H5V4h2v3h10V4h2v16z"/></svg>';
  const downloadSvgIcon = '<svg viewBox="0 0 24 24"><path d="M19 9h-4V3H9v6H5l7 7 7-7zM5 18v2h14v-2H5z"/></svg>';
  const buttonSpinnerHtml = '<div class="btn-spinner"></div>';
          ...
          els.resultDiv.innerHTML = `
            <div class="result-card">
              ...
              <div class="stats-grid">
                <div class="stat-item"><svg viewBox="0 0 24 24"><path d="M12 4.5C7 4.5 2.73 7.61 1 12c1.73 4.39 6 7.5 11 7.5s9.27-3.11 11-7.5c-1.73-4.39-6-7.5-11-7.5zM12 17c-2.76 0-5-2.24-5-5s2.24-5 5-5 5 2.24 5 5-2.24 5-5 5zm0-8c-1.66 0-3 1.34-3 3s1.34 3 3 3 3-1.34 3-3-1.34-3-3-3z"/></svg><span>${numberFormat(data.play_count)}</span></div>
                <div class="stat-item"><svg viewBox="0 0 24 24"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg><span>${numberFormat(data.digg_count)}</span></div>
                <div class="stat-item"><svg viewBox="0 0 24 24"><path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zM6 9h12v2H6V9zm8 5H6v-2h8v2zm4-6H6V6h12v2z"/></svg><span>${numberFormat(data.comment_count)}</span></div>
                <div class="stat-item"><svg viewBox="0 0 24 24"><path d="M18 16.08c-.76 0-1.44.3-1.96.77L8.91 12.7c.05-.23.09-.46.09-.7s-.04-.47-.09-.7l7.05-4.11c.54.5 1.25.81 2.04.81 1.66 0 3-1.34 3-3-1.34-3-3-3-3 1.34-3 3c0 .24.04.47.09.7L8.04 9.81C7.5 9.31 6.79 9 6 9c-1.66 0-3 1.34-3 3s1.34 3 3 3c.79 0 1.5-.31 2.04-.81l7.12 4.16c-.05.21-.08.43-.08.65 0 1.61 1.31 2.92 2.92 2.92 1.61 0 2.92-1.31 2.92-2.92s-1.31-2.92-2.92-2.92z"/></svg><span>${numberFormat(data.share_count)}</span></div>
              </div>
  ...
  window.toggleAllPhotos = function(select) {
    document.querySelectorAll('.photo-checkbox').forEach(cb => {
      ...
      if (parentItem) parentItem.classList.toggle('selected', select);
    });
  };
  ...
  window.downloadSelectedPhotos = async function(randomCode) {
    ...
    if (btn) {
      ...
      animInterval = setInterval(() => {
        dotCount = (dotCount % 3) + 1;
        btn.innerHTML = `${downloadSvgIcon}<span>Mengunduh${'.'.repeat(dotCount)}</span><span class="hd-badge">HD</span>`;
      }, 400);
    }
    try {
      for (let i = 0; i < checkboxes.length; i++) {
        ...
        try {
          ...
          a.click();
          setTimeout(() => { window.URL.revokeObjectURL(blobUrl); document.body.removeChild(a); }, 1000);
        } catch (err) {
          const a = document.createElement('a');
          a.href = imgUrl;
          a.download = filename;
  ...
  function executeReset() {
    ...
    if (els.realResetBtn) els.realResetBtn.style.display = 'none';
    if (els.submitBtn) { els.submitBtn.innerHTML = pasteSvgIcon; els.submitBtn.disabled = false; }
    if (els.resultDiv) els.resultDiv.innerHTML = '';
  }
  ...
  if (resetBtn) {
    ...
    resetBtn.addEventListener('touchend', function(e) { e.preventDefault(); executeReset(); });
  }
  ...
  window.downloadVideoFile = async function(videoUrl, filename, btn) {
    ...
    animInterval = setInterval(() => {
      ...
      btn.innerHTML = `${downloadSvgIcon}<span>Mengunduh${'.'.repeat(dotCount)}</span>${isHd ? '<span class="hd-badge">HD</span>' : ''}`;
    }, 400);
    try {
